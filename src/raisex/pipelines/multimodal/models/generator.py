...
def _get_multimodal_config() -> Dict[str, Any]:
    ...
    mm_cfg = config.get("multimodal_generator")
    ...
    try:
        ...
        if images:
            return llm.generate_multimodal(
                user_prompt, images=images, system=_get_system_prompt()
            )
...
async def generate_answer_async(
    ...
    try:
        ...
        if images:
            if hasattr(llm, "generate_multimodal_async"):
                return await llm.generate_multimodal_async(
                    user_prompt, images=images, system=system_prompt
                )
            return await asyncio.to_thread(
                llm.generate_multimodal, user_prompt, images=images, system=system_prompt
            )
