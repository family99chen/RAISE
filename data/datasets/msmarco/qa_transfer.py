...
def _extract_passages(item: Dict[str, Any]) -> List[str]:
    passages = item.get("passages")
    if not isinstance(passages, dict):
        return []
    texts = passages.get("passage_text")
    result: List[str] = []
    for text in _ensure_list(texts):
...
def _is_valid_item(item: Dict[str, Any]) -> bool:
    ...
    return bool(query and passages)
...
def _build_outputs(
    ...
    for idx, item in enumerate(qa_items):
        if not isinstance(item, dict):
            continue
        qid = item.get("query_id") or item.get("qid") or item.get("id") or str(idx)
        query = _normalize_text(item.get("query"))
        if not query:
            continue
        answers = [
            _normalize_text(ans)
            for ans in _ensure_list(item.get("answers"))
            if _normalize_text(ans)
        ]
        passages = _extract_passages(item)
        if not passages:
            continue

        qa.append({"id": str(qid), "query": query, "references": answers})
    ...
    for idx, item in enumerate(corpus_items):
        ...
        for text in passages:
            if text in seen:
                continue
            seen.add(text)
            corpus.append({"id": f"{qid}_{local_idx}", "content": text})
            local_idx += 1
...
def _pick_extra_items(
    ...
    if extra_count <= 0:
        return []
    qa_ids = {
        str(item.get("query_id") or item.get("qid") or item.get("id") or "")
    ...
    candidates = [
        ...
        if isinstance(item, dict)
        and str(item.get("query_id") or item.get("qid") or item.get("id") or "")
        not in qa_ids
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build QA/corpus from MS MARCO.")
    ...
    parser.add_argument("--subset", default="v1.1", help="Dataset subset: v1.1/v2.1.")
